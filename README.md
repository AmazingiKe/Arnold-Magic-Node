...
可以在插件的“设置 → AI 设置”中编辑接口、模型、协议、超时和大小限制。页面中的可选 API Key 只保留在当前 Maya 进程，并绑定到界面显示的 HTTPS origin；切换接口 origin 后不会复用旧密钥。保存设置本身不会发送网络请求。

也可以在启动 Maya 前给当前进程配置环境变量，密钥不会写入插件设置：
